...
const VANITY_STYLES = ["Modern", "Traditional", "Custom"];
...
const TOWEL_BAR_FINISHES = ["Chrome", "Black"];
...
export function StepTiledDryArea({ data, onUpdate, onNext, onPrev }: Props) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            {VANITY_STYLES.map((style) => {
              const isSelected =
                data.vanityStyle === style || (style === "Custom" && !VANITY_STYLES.slice(0, 2).includes(data.vanityStyle));
              return (
                <button
                  key={style}
                  type="button"
                  onClick={() => {
                    if (style === "Custom") {
                      if (VANITY_STYLES.slice(0, 2).includes(data.vanityStyle)) {
          ...
          {/* Custom Style Input */}
          {(!VANITY_STYLES.slice(0, 2).includes(data.vanityStyle)) && (
      ...
      {/* Mirror / Medicine Cabinet Choice */}
      <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-100 space-y-3">
        <div>
          <h3 className="text-xs font-bold text-slate-900">Mirror / Medicine Cabinet Choice</h3>
          <p className="text-[11px] text-slate-400">Select your mirror or medicine cabinet option</p>
        </div>
      ...
      {/* Lighting Choice */}
      <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-100 space-y-3">
        <h3 className="text-xs font-bold text-slate-900">Lighting Choice</h3>
        ...
        <div className="space-y-1.5 pt-3">
          <label className="block text-xs font-bold text-slate-900">Towel Bar Finish</label>
          <div className="flex gap-2">
              ...
              return (
                <button
                  ...
                  className={`rounded-xl px-5 py-2 text-xs font-semibold transition-all ${
                    isSelected ? "bg-[#C4A47C] text-white shadow-sm" : "bg-slate-100 text-slate-700 hover:bg-slate-200"
                  }`}
                >
                  {finish}
                </button>
